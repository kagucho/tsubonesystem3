    ...
    t.Run(`ServeMovedPermanently`, func(t *testing.T) {
      ...
      if recorder.Code != http.StatusMovedPermanently {
        t.Error(`invalid status code; expected`,
                http.StatusMovedPermanently, `, got `, recorder.Code)
      }
    ...
    t.Run(unknown.directory, func(t *testing.T) {
      ...
      t.Run(`ServeMovedPermanently`, func(t *testing.T) {
        ...
        t.Run(`success`, func(t *testing.T) {
          ...
          if resultLen := recorder.HeaderMap.Get(`Content-Length`)
             resultLen != `21` {
        ...
        t.Run(`failWrite`, func(t *testing.T) {
          ...
          if result := recorder.ResponseRecorder.HeaderMap.Get(`Content-Language`)
             result != `ja` {
            t.Errorf(`invalid Content-Language field in header; expected "ja", got %q`,
                     result)
          }
          ...
          if resultLen :=
               recorder.ResponseRecorder.HeaderMap.Get(`Content-Length`)
             resultLen != `21` {
            t.Errorf(`invalid Content-Length field in header; expected "TODO", got %q`,
                     resultLen)
          }

          if result := recorder.ResponseRecorder.HeaderMap.Get(`Location`)
             result != `https://kagucho.net/` {
            t.Errorf(`invalid Location field in header; expected "https://kagucho.net/", got %q`,
                     result)
          }

          if result := recorder.ResponseRecorder.Body.String()
             result != "https://kagucho.net/\n" {
            t.Errorf(`invalid body; expected "https://kagucho.net/\n", got %q`,
                     result)
          }
