...
// ListServeHTTP serves the functionality of API v0 to list officers via HTTP.
func ListServeHTTP(writer http.ResponseWriter, request *http.Request,
                    dbInstance db.DB, claim authorizer.Claim) {
  officers, errors := dbInstance.QueryOfficers()
  common.ServeJSONChan(writer, officers, officers, errors, http.StatusOK)
}
