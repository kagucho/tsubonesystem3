...
func TestParseQuery(t *testing.T) {
  for _, test := range [...]struct{
    ...
    t.Run(test.description, func(t *testing.T) {
      if result := parseQuery(test.routeQuery)
          ...
          t.Run(`unescapedFragment`, func(t *testing.T) {
            ...
            t.Run(`withQuery`, func(t *testing.T) {
              ...
              if result := recorder.HeaderMap.Get(`Location`)
                 result != `https://kagucho.net/private` {
            ...
            []queryTest{
              {
                ...
                graph{
                  []property{
                    {`og:description`, `神楽坂一丁目通信局の局員、 !\%_1"#の詳細情報です。`},
                    {`og:title`, `TsuboneSystem  !\%_1"#の詳細情報`},
                ...
                graph{
                  []property{
                    {`og:description`, `神楽坂一丁目通信局の局員、 !\%*1"#の詳細情報です。`},
                    {`og:title`, `TsuboneSystem  !\%*1"#の詳細情報`},
