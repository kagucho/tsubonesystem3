...
describe("Client", () => {
  ...
  describe("constructor", () => it("should freeze", () => {
    const result = Object.isFrozen(client);

    if (!result)
      throw new Error("Object.isFrozen(client) returns " +
  ...
  describe("recoverSession", () => {
    ...
    it("should return a successful promise", function(done) {
      signinPromise.then(() => recoverPromise.done(done), () => this.skip());
    });

    it("should allow memberDetail to return a successful promise", function() {
      return recoverPromise.then(() => recovering.memberDetail("1stDisplayId"),
                                 () => this.skip());
    });
  });
