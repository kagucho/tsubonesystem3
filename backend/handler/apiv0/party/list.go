...
func ListServeHTTP(writer http.ResponseWriter, request *http.Request,
	...
	parties := context.DB.QueryParties(claim.Sub)
	common.ServeJSON(writer, parties, http.StatusOK)
}

func ListnamesServeHTTP(writer http.ResponseWriter, request *http.Request,
	context context.Context, claim authorizer.Claim) {
	common.ServeJSON(writer, context.DB.QueryPartyNames(), http.StatusOK)
}
