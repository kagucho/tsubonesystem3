...
func TestCustomizedResponseWriter(t *testing.T) {
	...
	for _, test := range [...]struct {
		...
		{
			`validError`,
			http.StatusNotFound, `test/unknown/na/error/404`,
		},
		{
			`invalidError`,
		...
		t.Run(test.description, func(t *testing.T) {
			...
			customizedWriter := customizedResponseWriter{
				recorder, &hijacked, fileError}
...
func (file File) TestServeHTTP(t *testing.T) {
	for _, test := range [...]struct {
		...
		t.Run(test.request, func(t *testing.T) {
			...
			if result := recorder.HeaderMap.Get(`Content-Language`); result != test.lang {
	...
	t.Run(`/license/?redundant_query`, func(t *testing.T) {
		...
		if result := recorder.HeaderMap.Get(`Location`); result != `http://kagucho.net/license` {
