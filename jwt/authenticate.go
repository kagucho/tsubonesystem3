...
func (context JWT) validateHeader(encoded string) Error {
  ...
  if decoder.More() {
    return Error{ errors.New(`header contains something superfluous`),
                  `https://tools.ietf.org/html/rfc7159` }
  }

  if decoded.Alg != context.authority.Alg() {
    return Error{ fmt.Errorf(`expected alg "HS256", header says %q`,
                             decoded.Alg),
...
func validClaim(encoded string) (Claim, Error) {
  ...
  var decoded claim
  if decodeError := decoder.Decode(&decoded); decodeError != nil {
    return Claim{}, Error{decodeError, `https://tools.ietf.org/html/rfc7159`}
  }

  if decoder.More() {
    return Claim{}, Error{errors.New(`claim contains something superfluous`),
                           `https://tools.ietf.org/html/rfc7159`}
  }
  ...
  return Claim{decoded.Sub, decoded.Scope, duration}, Error{}
}
...
func (context JWT) Authenticate(jwt string) (Claim, Error) {
  ...
  if len(splited) != 3 {
    return Claim{},
           Error{fmt.Errorf(`expected 3 parts, got %v parts`, len(splited)),
  ...
  if decodeError != nil {
    return Claim{}, Error{decodeError,
  ...
  if !hmac.Equal(hash.Sum(nil), signature) {
    return Claim{}, Error{errors.New(`invalid signature`),
